...
def draw_plot_func(dictionary, n_classes, window_title, plot_title, x_label, output_path, to_show, plot_color,
    ...
    if true_p_bar != "":
        ...
        for i, val in enumerate(sorted_values):
            ...
            t = plt.text(val, i, str_val, color=[0, 0, 0], va='center', fontweight='bold', fontdict={"size": 14})
    ...
    # set axis titles
    # plt.xlabel('classes')
    plt.xlabel(x_label, fontdict={'family': 'Arial', 'size': 16}, fontweight='bold')
...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path='./map_out'):
    ...
    RESULTS_FILES_PATH = os.path.join(path, 'results_%.3f'%MINOVERLAP)
    ...
    for txt_file in tqdm(ground_truth_files_list):
    ...
    if draw_plot:
        window_title = "mAP"
        # plot_title = "mAP = {0:.3f}%".format(mAP * 100)
        plot_title = "mAP=%.3f" % mAP
        x_label = "Average Precision"
        output_path = RESULTS_FILES_PATH + "/mAP_%.1f.pdf"%MINOVERLAP
        to_show = True
        plot_color = 'royalblue'
        draw_plot_func(
            ap_dictionary,
